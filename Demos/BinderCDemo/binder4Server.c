                 ...
                 uintptr_t ptr, size_t size, binder_handler func)
{
    ...
    while (ptr < end) {
        ...
        case BR_TRANSACTION_COMPLETE:
            ALOGW("进入 BR_TRANSACTION_COMPLETE");
            break;
        case BR_INCREFS:
            ALOGW("进入 BR_TRANSACTION_COMPLETE");
        case BR_ACQUIRE:
        ...
        case BR_TRANSACTION_SEC_CTX:
            ALOGW("进入 BR_TRANSACTION_SEC_CTX");
        case BR_TRANSACTION: {
            ALOGW("进入 BR_TRANSACTION 处理流程");
        ...
        case BR_FAILED_REPLY:
            ALOGW("进入 BR_FAILED_REPLY");
            r = -1;
            break;
        case BR_DEAD_REPLY:
            ALOGW("进入 BR_DEAD_REPLY");
            r = -1;
            break;
        default:
             ALOGW("进入 default");
                ...
                uint32_t target, uint32_t code)
{
    ...
    ALOGW("发送的 cmd 为 %u ", writebuf.cmd);
    ...
    for (;;) {
        ...
        if (res < 0) {
            ...
            ALOGW("binder call ioctl 调用失败");
            goto fail;
        } else {
            ALOGW("binder call ioctl 调用成功");
        }

        ALOGW("binder call 开始解析数据");
        res = binder_parse(bs, reply, (uintptr_t) readbuf, bwr.read_consumed, 0);
        if (res == 0) {
            ALOGW("binder call 数据解析调用成功");
            return 0;
        }
        if (res < 0) {
            ALOGW("binder call 数据解析调用失败");
            goto fail;
        }
        }
